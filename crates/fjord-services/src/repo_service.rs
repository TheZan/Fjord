...
mod tests {
    ...
    use fjord_domain::{CommitPage, FileDiff, LogCursor, RepoStatus, RepositoryEntry, Workspace, WorkspaceId};
    ...
    impl WorkspaceStore for FakeStore {
        async fn list_workspaces(&self) -> Result<Vec<Workspace>, StoreError> { Ok(vec![]) }
        async fn create_workspace(&self, _name: &str) -> Result<Workspace, StoreError> { unimplemented!() }
        async fn rename_workspace(&self, _id: WorkspaceId, _name: &str) -> Result<Workspace, StoreError> { unimplemented!() }
        async fn reorder_workspaces(&self, _ids: &[WorkspaceId]) -> Result<(), StoreError> { unimplemented!() }
        async fn delete_workspace(&self, _id: WorkspaceId) -> Result<(), StoreError> { unimplemented!() }
        async fn list_repositories(&self, _workspace_id: WorkspaceId) -> Result<Vec<RepositoryEntry>, StoreError> { unimplemented!() }
        async fn get_repository(&self, id: RepositoryId) -> Result<RepositoryEntry, StoreError> {
            if id == self.repo.id { Ok(self.repo.clone()) } else { Err(StoreError::RepositoryNotFound(id)) }
        }
        async fn add_repository(&self, _workspace_id: WorkspaceId, _name: &str, _path: &Path) -> Result<RepositoryEntry, StoreError> { unimplemented!() }
        async fn remove_repository(&self, _id: RepositoryId) -> Result<(), StoreError> { unimplemented!() }
    }
    ...
    impl GitBackend for FakeGit {
        async fn status(&self, _repo: &RepoPath) -> Result<RepoStatus, GitError> { unimplemented!() }
        ...
        async fn commit(&self, _repo: &RepoPath, _message: &str) -> Result<String, GitError> { unimplemented!() }
        async fn fetch(&self, _repo: &RepoPath, _remote: &str) -> Result<(), GitError> { unimplemented!() }
        async fn pull(&self, _repo: &RepoPath) -> Result<(), GitError> { unimplemented!() }
        async fn push(&self, _repo: &RepoPath, _refspec: &str) -> Result<(), GitError> { unimplemented!() }
    }
    ...
    async fn unknown_repo_id_is_reported_before_touching_git() {
        ...
        let service = RepoService::new(
            ...
            Arc::new(FakeGit { seen_path: Mutex::new(None) }),
        );
