...
- ✅ **One backend workspace-health projection.** `WorkspaceService::get_workspace_health` derives ordered multi-condition `RepoHealth` values in O(repositories) from the status cache plus cached operation/error observations. Dashboard and sidebar consume the same `needs_attention` value; no frontend surface redefines attention, and the query performs no Git reads.
...
| Phase 10 — Daily-driver integration, advanced workflows & workspace | 🚧 in progress: branch merge (`P10-MERGE-01`–`03`), Working Changes file actions (`P10-WC-01`–`06`), multi-selection and all batch actions (`P10-WC-MULTI-01`–`03`), stash management (`P10-STASH-01`–`06`), and backend-owned workspace repository health (`P10-08`) are complete. Expected-branch settings (`P10-09`) and health filter chips (`P10-10`) remain open. See [`tasks.md`](tasks.md) §"Phase 10 execution order". |
