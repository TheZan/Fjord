...
fixes and Phase 9.5 implementation are complete. The v0.1 public launch remains
blocked by the exact-candidate evidence recorded in
[`v0.1-launch-decision.md`](v0.1-launch-decision.md); Phases 10–11 remain
designed. Their contracts live in [`specs/`](specs/) and their goals in §15.
...
- ✅ **One backend workspace-health projection.** `WorkspaceService::get_workspace_health` derives ordered multi-condition `RepoHealth` values in O(repositories) from the status cache plus cached operation/error observations. Dashboard, sidebar, and the six shared Overview/All Repositories health filters consume that projection; no frontend surface redefines attention or expected-branch drift, and filter interaction performs no Git or IPC reads.
...
- ✅ **Errors**: `thiserror` typed errors per crate (`GitError`, `StoreError`, service-level enums), mapped at the `commands` boundary to a small serializable `AppError { code, message, diagnostics?, paths?, stash_ref? }` — the frontend switches on stable fields and never parses Rust `Display` strings. Error *messages* shown to the user go through the i18n catalog.
...
| Phase 12 — Merge completeness & conflict resolution | ✅ done: `--no-ff` with an editable merge message (`P12-MERGE-01`), wider sources/unrelated histories (`P12-MERGE-02`), in-app conflict resolution from the live index (`P12-MERGE-03`, [`specs/conflict-resolution.md`](specs/conflict-resolution.md)), **Update `{{branch}}` from `{{source}}`**, a fast-forward of a branch that is not checked out (`P12-MERGE-04`, [`specs/branch-merge.md`](specs/branch-merge.md) §10.3), and the `-X ours`/`-X theirs` strategy option worded by ref behind the merge dialog's Advanced disclosure (`P12-MERGE-05`, §10.4) |
...
| Partial staging corrupts uncommitted work | Patches are applied by `git apply`, never by a hand-written index writer. Every selection is verified against a freshly recomputed diff digest; concurrent Fjord operations, standard Git commands, and conforming Git clients serialize through the in-process lock plus Git's resolved per-worktree index/ref locks. Direct raw index replacement that ignores Git locks is unsupported, because no portable filesystem compare-and-swap can make fingerprint verification and publication indivisible. Worktree-only editor races remain a separate documented limitation ([`specs/working-tree-and-diff.md`](specs/working-tree-and-diff.md) §1). |
...
- [`specs/`](specs/) — per-subsystem contracts. One spec owns each contract; other documents cross-reference it rather than restating it:
  ...
  - Implemented: [`branch-merge.md`](specs/branch-merge.md) (`P10-MERGE-01`–`P10-MERGE-03`: local, remote-tracking, and squash merge); its integration preflight is the owner rebase reuses. Implemented: [`working-tree-and-diff.md`](specs/working-tree-and-diff.md) §6 (`P10-WC-01`–`P10-WC-06`) — the single owner of the Working Changes file context menu; its implemented §7 (`P10-WC-MULTI-01`–`P10-WC-MULTI-03`) is the single owner of the Working Changes **selection model and batch actions**, which four non-stash actions and one stash action all read from.
  - Implemented through `P10-STASH-06`: [`stash-management.md`](specs/stash-management.md) — the single owner of stash product, domain, and action semantics; `working-tree-and-diff.md` §6, `ui-shell.md`, `ipc-commands.md`, and `repository-safety.md` §3 reference it rather than restating it. Designed: [`workspace-workflows.md`](specs/workspace-workflows.md) (Phase 10) and [`release-hardening.md`](specs/release-hardening.md) (Phase 11). Implemented through `P12-MERGE-03`: [`conflict-resolution.md`](specs/conflict-resolution.md) (Phase 12) — the single owner of resolving a conflicted path for every operation that produces one, referenced by `branch-merge.md`, `repository-safety.md`, and `working-tree-and-diff.md` rather than restated in them.
...
| 10 — Daily-driver integration, advanced workflows & workspace | Close the remaining daily-driver gaps — merge initiation, Working Changes file actions, multi-file selection, stash management as a first-class workflow — then turn the workspace into the competitive advantage: worktrees, rebase, remotes, workspace health, expected branch, filters. Complete. | [`specs/branch-merge.md`](specs/branch-merge.md), [`specs/working-tree-and-diff.md`](specs/working-tree-and-diff.md) §6, [`specs/stash-management.md`](specs/stash-management.md), [`specs/workspace-workflows.md`](specs/workspace-workflows.md) | 9 (merge, rebase, and worktree removal depend on operation state and preflights; the file menu depends on Phase 8 patch/discard semantics; stash pop/drop depend on the destructive preflight contract); 9.5 (extends its minimal add/connect-remote slice into full CRUD without blocking v0.1) |
...
| 12 — Merge completeness & conflict resolution | Finish the integration workflow Phase 10 started: an explicit merge commit with a message the user wrote, any ref as a source, and — the phase's real value — **resolving a conflicted file inside Fjord** instead of in a terminal. Admission rule: today a conflict, or a `--no-ff` release merge, still sends the user out of Fjord, and the user who leaves finishes the whole task outside. `P12-MERGE-04` (shipped) additionally admits a *fast-forward update* of a branch that is not checked out, on its own admission argument: it removes the reason to open a terminal for `git fetch origin main:main` — updating a local integration branch without disturbing the branch being worked on, a daily action in a multi-repository workflow — and it is not a merge, cannot conflict, and moves one ref by compare-and-swap. A true merge into a non-checked-out branch, and a built-in three-way editor, stay out of scope. | [`specs/branch-merge.md`](specs/branch-merge.md) §10, [`specs/conflict-resolution.md`](specs/conflict-resolution.md) | 9 (operation state and Continue/Abort), 10 (the shipped merge, squash, rebase and worktree contracts it extends); independent of 11 |
