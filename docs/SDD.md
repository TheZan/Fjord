...
- ⚠️ **Content Security Policy**: `tauri.conf.json` currently sets `csp: null`, disabling CSP entirely. Target: a strict CSP (self-only script/style sources, no remote origins) so a markup-injection bug cannot escalate to IPC access (`P4-01`).
