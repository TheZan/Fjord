...
| Phase 10 — Daily-driver integration, advanced workflows & workspace | 🚧 in progress: branch merge (`P10-MERGE-01`–`03`), Working Changes file actions (`P10-WC-01`–`06`), multi-selection and batch Stage/Unstage plus `Stash N files…` (`P10-WC-MULTI-01`–`02`), and stash management through commit-graph markers and the read-only inspector (`P10-STASH-01`–`05`) are complete. Stash actions (`P10-STASH-06`) plus batch Discard/multi-file patch export (`P10-WC-MULTI-03`) remain designed; later workspace work remains designed in [`specs/workspace-workflows.md`](specs/workspace-workflows.md). See [`tasks.md`](tasks.md) §"Phase 10 execution order". |
