...
| Stash | ⚠️ `create_stash` now owns both conventional whole-repository and exact selected-path creation (`P10-STASH-02`), alongside `get_stashes`, exact stash-entry consumption facts, and named tracked-plus-untracked stash-and-checkout that never auto-pops. Pop still runs through `execute_destructive_action`; the frontend passes a literal `0`, so only the top entry is reachable, and `DestructiveAction::StashPop { index }` still keys on a moving position until `P10-STASH-06`. There is no Drop yet. |
