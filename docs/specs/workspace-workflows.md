...
| Filters | ✅ Overview and All Repositories share the six persisted health filters (needs attention, dirty, ahead, behind, conflicts, wrong branch). Health filters compose with OR; All Repositories text search composes with the health result using AND. Filtering is client-side over the loaded `RepoHealth` map, so WrongBranch is one click away without Git or IPC work. |
