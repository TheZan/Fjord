...
| Diff rendering | ⚠️ Unified and split modes share the accumulated diff and virtualized renderer (`FileDiffView.tsx`). Split rows pair deletion/addition runs and pad the shorter side; the persisted header toggle remeasures the virtualizer and restores its logical scroll anchor. Syntax highlighting and thresholded word-level LCS run in a dedicated worker over visible rows only, after plain paint, and skip over-budget inputs. Word ranges are presentational and do not change patch coordinates. Show / ignore trailing / ignore all whitespace modes are backend diff options; ignored modes disable partial patch actions with a reason. Huge-diff UX remains open. |
