...
| Diff rendering | ⚠️ Unified and split modes share the accumulated diff and virtualized renderer (`FileDiffView.tsx`). Split rows pair deletion/addition runs and pad the shorter side; the persisted header toggle remeasures the virtualizer and restores its logical scroll anchor. Syntax highlighting runs in a dedicated worker over visible rows only, after plain paint, and skips unknown/over-budget inputs. Show / ignore trailing / ignore all whitespace modes are backend diff options; ignored modes disable partial patch actions with a reason. Word diff remains open. |
