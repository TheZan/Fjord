...
| `merge_branch` | `{ repo_id, source, mode, dirty_policy, allow_unrelated_histories, message?, strategy_option?, operation_id? }` | `MergeResult` | Cancellable merge through system Git; supports local branches, remote-tracking refs, tags, and raw commit ids. The unrelated-histories flag is honored only for an `Unrelated` preflight. `message` (≤ 4 KiB, no NUL, not blank; line endings normalized to LF) is passed to Git as one `-m` argument and is the exact committed text, including after a conflicted merge is continued; `null` keeps Git's default message. `strategy_option` (`preferTarget` / `preferSource`, default `null`) adds `-X ours` / `-X theirs` as two arguments — conflicting lines take that side and the other side's are discarded; `null` leaves conflicts to the user (`P10-MERGE-01`, `P10-MERGE-02`, `P12-MERGE-01`, `P12-MERGE-02`, `P12-MERGE-05`) |
...
| `start_interactive_rebase` | `{ repo_id, preflight: RebasePreflight, steps: RebaseTodoStep[], dirty_policy: MergeDirtyPolicy, operation_id? }` | `RebaseResult` | Revalidates the preflight and the edited todo list (same commit set, no empty reword/squash message, no leading fixup/squash) under the write lock, then writes Git's todo file and starts `git rebase --interactive`. Reword/squash messages are applied by Fjord itself at synthetic pause points it inserts into the todo list — never through an interactive editor — and draining them is folded into `continue_operation`, so a real conflict still surfaces through the ordinary Phase 9 controls and Abort restores the original history. Invalid todos fail `rebase_todo_invalid` before anything is written. (`P10-11`) |
| `squash_merge_branch` | `{ repo_id, source, dirty_policy, allow_unrelated_histories, operation_id? }` | `SquashMergeResult` | Cancellable `merge --squash` through system Git; shares `get_merge_preflight`'s blockers, dirty-tree rules, and unrelated-history acknowledgement. Stages the combined diff (or leaves it conflicted) without a merge commit and without moving any ref, so a conflict is a live index read rather than a `RepoOperationState`, and any outcome can be discarded with a plain Reset (Hard) to the returned `targetCommit` (`P10-MERGE-03`, `P12-MERGE-02`) |
...
| `get_conflicts` | `{ repo_id }` | `GenerationEnvelope<ConflictSet>` | Read-only under the repository read lock. The live index's conflicted paths (`git2::Index::conflicts()`, never `RepoOperationState`), so it serves a conflicted `merge --squash` or `stash apply` whose operation state is `Normal`. Each entry carries its present stages (object id, mode, size — never content) and the `ConflictKind` derived from them; bounded at 1000 entries ordered by byte path with `truncated` and the exact `total`. `sides` names both sides by the refs Git means (`MERGE_HEAD`, `rebase-*/onto` + `head-name`, `CHERRY_PICK_HEAD`, `REVERT_HEAD`, the recorded squash source or applied stash), with `inverted` for the rebase family. Validated by the `working_tree` generation ([`conflict-resolution.md`](conflict-resolution.md), `P12-MERGE-03`) |
| `resolve_conflict` | `{ repo_id, path, resolution, allow_markers?, expected_generations, operation_id? }` | `ConflictSet` | Operation kind `resolve-conflict`. Under the write lock: a stale `expected_generations`, a path that is not a current conflict, or a path that could leave the worktree fails `preflight_stale` before anything runs; a resolution the path's `ConflictKind` does not offer fails `conflict_resolution_not_applicable`; `markResolved` without `allow_markers` scans the first 1 MiB of the worktree file and fails `conflict_markers_present` (with `paths` and `line`). Then runs `checkout --ours\|--theirs -- <path>` + `add -- <path>`, `rm -f -- <path>`, or `add -- <path>` through system Git with `GIT_LITERAL_PATHSPECS=1`, the path always one argument. Advances `working_tree` alone once Git has launched; a failed step is `conflict_resolution_failed` and leaves the path conflicted. Returns the refreshed set (`P12-MERGE-03`) |
...
| `push_tag` | `{ repo_id, tag, remote, operation_id? }` | — | Pushes only an existing local tag to one explicitly selected configured remote with the exact `refs/tags/<tag>:refs/tags/<tag>` refspec. Never forces or changes branch upstream. |
...
Phase 12 (`P12-MERGE-01`–`05`) planned three new commands. `get_conflicts` and
`resolve_conflict` have shipped with `P12-MERGE-03` and moved to the table above;
its two extended payloads have shipped too (below). One remains planned:
...
`update_branch_fast_forward` carries `expected_tip` because its `update-ref` is a
compare-and-swap on the old value; it advances `refs` and `history` and never
`working_tree`.
