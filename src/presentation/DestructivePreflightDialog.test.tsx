...
import type { DestructiveAction, DestructivePreflight } from "@/domain/generated";
...
function preflight(workingTree: number, count = 3, blockers: string[] = []): DestructivePreflight {
  return {
    action,
...
describe("DestructivePreflightDialog", () => {
  it("re-runs preflight at confirmation and never executes stale facts", async () => {
    const loadPreflight = vi
      .fn()
      .mockResolvedValueOnce(preflight(1, 3))
      .mockResolvedValueOnce(preflight(2, 5))
      .mockResolvedValueOnce(preflight(2, 5));
    ...
    fireEvent.click(confirm);
    await waitFor(() => expect(onConfirm).toHaveBeenCalledWith(preflight(2, 5).generations));
